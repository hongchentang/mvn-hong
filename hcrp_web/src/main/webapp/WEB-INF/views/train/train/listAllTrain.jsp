...
<form id="listAllTrainForm" action="${ctx}/train/train/listAllTrain.do?tabId=${param['tabId']}" method="post">
	<div class="easyui-panel" title="" data-options="" style="width: 100%;">
		<div class="datagrid-toolbar">
			<table cellspacin="0" cellpadding="3px">
				<tr>
					...
					<td>	
						<a href="javascript:void(0)" class="easyui-linkbutton" onclick="easyuiUtils.ajaxPostFormForPanel('listAllTrainForm','${param['tabId']}')" ><i class="fa fa-search"></i> 查询</a>
		...
		<div>
			<table id="listAllTrainTable" rownumbers="true" pagination="true" checkOnSelect="true" selectOnCheck="true" nowrap="true" singleSelect="true">
				<thead>
					<tr>
						...
						<th width="100" data-options="field:'chakan'">评价查看</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${listTrain }" var="train">
						<tr>
							...
							<td> 
								<c:if test="${train.courseEvaluaCount==0&&train.teachingEvaluaCount==0}">
					           		 暂无评价
					           	</c:if>
					           	<c:if test="${train.courseEvaluaCount>0&&train.teachingEvaluaCount>0}">
					           		<a href="javascript:void(0)" class="easyui-linkbutton" onclick="readTrain('${train.id}')" title="点击查看评价">查看评价</a>
					           	</c:if>
							</td>
						</tr>
					</c:forEach>
			...
			<jsp:include page="/jsp/common/include/manage_page_table.jsp">
				<jsp:param value="listAllTrainForm" name="pageForm" />
				<jsp:param value="listAllTrainTable" name="tableId" />
...
<script>
function viewTrain(){
	var objects = $('#listAllTrainTable').datagrid('getSelections');
	if(objects.length == 0){
...
function readTrain(valueId){
	 
	openWindow('readTrainWindow','查看培训班评价信息',0,600,'${ctx}/train/train/readEvaluate.do?id='+valueId,true);
}
