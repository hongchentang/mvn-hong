...
<div style="margin: 5px;margin-bottom: 27px">
		<form id="listUnPassForm" action="${ctx}/train/channel/listUnPass.do" method="post">
			<div class="easyui-panel" title="" data-options="" style="width: 100%;">
				<div class="datagrid-toolbar" float:right>
					<table cellspacin="0" cellpadding="3px">
						<tr>
							...
							<td>
								<a href="javascript:void(0)" class="easyui-linkbutton" onclick="easyuiUtils.ajaxPostFormForPanel('listUnPassForm','viewProListN')" iconCls="fa fa-search">查询</a>
							</td>
						</tr>	
					</table>
					<table cellspacing="0" cellpadding="0">
							<tbody>
								<tr>
									<td><a onclick="showDetail_unpass()" href="javascript:void(0)" class="easyui-linkbutton">查看详细</a>	</td>
				...
				<div>
					<table id="listUnPassTable" rownumbers="true" pagination="true" checkOnSelect="true" selectOnCheck="true" nowrap="true" singleSelect="true">
						<thead>
							<tr>
					...
					<jsp:include page="/jsp/common/include/manage_page_table.jsp">
						<jsp:param value="listUnPassForm" name="pageForm" />
						<jsp:param value="listUnPassTable" name="tableId" />
						<jsp:param value="easyui" name="type" />
						<jsp:param value="viewProListN" name="divId" />
					</jsp:include>
				</div>
			</div>
		</form>
...
<script>
function showDetail_unpass(){
	var objects = $('#listUnPassTable').datagrid('getSelections');
	if(objects.length == 0){
			...
			$.ajax({
				...
				success:function(data){
					...
					if (parseInt(responseCode) == 200) {
						...
						jQuery("#viewProListN").panel('refresh');
					} else {
