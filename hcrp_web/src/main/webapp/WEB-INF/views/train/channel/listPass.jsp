...
<div style="margin: 5px;margin-bottom: 27px">
	<!-- <div class="easyui-panel" title="课程管理" style="width: 100%;" > -->
		<form id="listPassForm" action="${ctx}/train/channel/listPass.do" method="post">
				...
				<div class="datagrid-toolbar" float:right>
					<table cellspacin="0" cellpadding="3px">
						<tr>
							...
							<td>
								<a href="javascript:void(0)" class="easyui-linkbutton" onclick="easyuiUtils.ajaxPostFormForPanel('listPassForm','viewProListY')" iconCls="fa fa-search">查询</a>
							</td>
						</tr>	
					</table>
					<table cellspacing="0" cellpadding="0">
							<tbody>
								<tr>
									...
									<td>&nbsp;<a onclick="show_ask()" href="javascript:void(0)" class="easyui-linkbutton">邀请学员</a>	</td>
								</tr>
							</tbody>
						</table>
				</div>
				<div>
					<table id="listPassTable" rownumbers="true" pagination="true" checkOnSelect="true" selectOnCheck="true" nowrap="true" singleSelect="true">
						<thead>
							<tr>
					...
					<jsp:include page="/jsp/common/include/manage_page_table.jsp">
						<jsp:param value="listPassForm" name="pageForm" />
						<jsp:param value="listPassTable" name="tableId" />
						<jsp:param value="easyui" name="type" />
						<jsp:param value="viewProListY" name="divId" />
					</jsp:include>
				</div>
			</div>
		</form>
...
function showDetail_pass(){
	var objects = $('#listPassTable').datagrid('getSelections');
	if(objects.length == 0){
...
function show_ask(){
	...
	var id = objects[0].id;
	openWindow('showAskWindow','邀请学员',600,200,'${ctx}/train/channel/showAsk.do?paramMap[projectId]='+id,true); 
}
			...
			$.ajax({
				...
				success:function(data){
					...
					if (parseInt(responseCode) == 200) {
						...
						jQuery("#viewProListY").panel('refresh');
					} else {
