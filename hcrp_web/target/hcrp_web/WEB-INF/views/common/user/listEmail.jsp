...
<div id="customeruser_search">
	<div style="margin: 10px">    
		姓名:                                                                   
		<input class="easyui-textbox" type="text" name="paramMap[userName]" value="${paramMap.userName}"></input>
	            邮箱:                                                                   
		<input class="easyui-textbox" type="text" name="paramMap[userEmail]" value="${paramMap.userEmail}"></input>
		所属公司:                                                                   
		<input class="easyui-textbox" type="text" name="paramMap[companyName]" value="${paramMap.companyName}"></input>
...
<!-- 		<button id="user_resetPassword" type="button" class="easyui-linkbutton"><span class="fa fa-undo"></span> 密码重置</button>
 ...
 <table id="user_list" title="客户邮箱管理"	
    ...
    singleSelect="true" checkOnSelect="true" selectOnCheck="true">
			<thead>
				...
				<%-- 	<th field="roleCode" data-options="hidden:true">角色代码</th>
					...
					<th field="companyName" width="25">所属公司</th>
					<th field="mobilePhoe" width="25">联系方式</th>
	...
	getCurrentTab().find('#user_editBtn').linkbutton({    
		onClick:function(){
			...
			if(userRow==null){
				...
				easyuiUtils.openWindow('showAddDialog','编辑用户',900,400,'${ctx }/common/user/custedit.do?id='+userRow.id,true);
			}
		}
	}); 
	getCurrentTab().find('#user_delBtn').linkbutton({    
		onClick:function(){
			...
			if(userRow==null){
				...
				easyuiUtils.confirmPostUrl('是否确定删除‘'+userRow.userName+'’用户?','${ctx }/common/user/deletes.do?user.id='+userRow.id);
			}
		}
	}); 
