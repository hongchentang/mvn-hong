...
<form id="studentForm" name="studentForm" action="${ctx}/common/user/employees/save.do" method="post" enctype="multipart/form-data">
		...
		<tbody>
			...
			<tr>
				...
				<td colspan="3">
					<select name="userType" id="userType" class="easyui-validatebox"  style="width:300px;" required></select>
                             ...
                             jQuery("#studentForm").find("#userType").combotree({
								...
								onLoadSuccess:function(){
									...
									if(value!=''){
										  jQuery("#studentForm").find("#userType").combotree('setValues',value);
									}
								 }
							}); 
                            </script>
				...
				<c:otherwise>
					<tr>
						...
						<td colspan="3">
							...
							<script type="text/javascript">
								jQuery('#studentForm').find('#deptId').combobox({    
								    ...
								    onChange:function(newValue, oldValue) {
								    	...
								    	if(newValue!=undefined&&""!=newValue) {
								    		...
								    		if(isVirtual=="1") {//其它机构，展示单位名称让用户填写
								    			$("#belongDeptName","#studentForm").textbox();
								    			$("#belongDeptName","#studentForm").textbox("enableValidation");
								    			$(".isVirtual_1").show();
								    		} else {
								    			$("#belongDeptName","#studentForm").textbox();
								    			$("#belongDeptName","#studentForm").textbox("setValue","");
								    			$("#belongDeptName","#studentForm").textbox("disableValidation");
								    			$(".isVirtual_1").hide();
								    		}
								    	}
								    }
								...
								jQuery('#studentForm').find('#regionsCode').combotree({
								    ...
								    onLoadSuccess:function(){
								    	if('${user.regionsCode}'!=''){
								    		jQuery('#studentForm').find('#regionsCode').combotree('setValue','${user.regionsCode}');
								    		if('${user.deptId}'!=''){
									    		jQuery('#studentForm').find('#deptId').combobox('setValue','${user.deptId}');
									    	}
										}
								    },
								    onChange:function(newValue, oldValue) {
								    	if(newValue!=undefined&&""!=newValue) {
									    	...
									    	jQuery('#studentForm').find('#deptId').combobox("reload",url);
									    	jQuery('#studentForm').find('#deptId').combobox('setValue','');
								    	}
								    }
								}); 
							</script>
					...
					<tr class="isVirtual_1">
						...
						<td><span style="color:red">*</span>所属单位地址</td>
						<td>
							<input type="text" class="easyui-textbox" id="corrAddress" name="corrAddress" required value="${user.corrAddress}"/>
						</td>
					</tr>
				</c:otherwise>
			</c:choose>
			...
			<tr>
				...
				<td>
					...
					<script type="text/javascript">
							jQuery('#studentForm').find('#hometownCity').combobox({
							    url:'${ctx}/common/regions/getNextRegionsByCode.do?regionsCode=${user.hometownProvince}',
							    valueField:'id',
							    textField:'text'
							});
							jQuery('#studentForm').find('#hometownProvince').combobox({
							    ...
							    onLoadSuccess:function(){
							    	if('${user.hometownProvince}'!=''){
							    		jQuery('#studentForm').find('#hometownProvince').combobox('setValue','${user.hometownProvince}');
									}
							    	if('${user.hometownCity}'!=''){
							    		jQuery('#studentForm').find('#hometownCity').combobox('setValue','${user.hometownCity}');
							    	}
							    },
							    onChange:function(newValue, oldValue) {
							    	if(newValue!=undefined&&""!=newValue) {
							    		var url = '${ctx}/common/regions/getNextRegionsByCode.do?regionsCode='+newValue;
								    	jQuery('#studentForm').find('#hometownCity').combobox("reload",url);
								    	jQuery('#studentForm').find('#hometownCity').combobox('setValue','');
							    	}
							    }
			...
			<tr>
				...
				<td colspan="3">
					...
					<input type="text" class="easyui-textbox" name="currentStreet" id="currentStreet" value="${user.currentStreet}" style="width:220px"/>
                             <script type="text/javascript">
                             jQuery('#studentForm').find('#currentCounties').combobox({
  							    url:'${ctx}/common/regions/getNextRegionsByCode.do?regionsCode=${user.currentCity}',
  							    valueField:'id',
  							    textField:'text'
  							 });
                              jQuery('#studentForm').find('#currentCity').combobox({
  							    ...
  							    onChange:function(newValue, oldValue) {
  							    	if(newValue!=undefined&&""!=newValue) {
  							    		var url = '${ctx}/common/regions/getNextRegionsByCode.do?regionsCode='+newValue;
  								    	jQuery('#studentForm').find('#currentCounties').combobox("reload",url);
  								    	jQuery('#studentForm').find('#currentCounties').combobox('setValue','');
  							    	}
  							    }
  							 });
                              jQuery('#studentForm').find('#currentProvince').combobox({
  								url:'${ctx}/common/regions/getNextRegionsByCode.do?regionsCode=${ipanthercommon:getChinaRegionsCode()}',
  							    valueField:'id',
  							    textField:'text',
  							    parentField:'pid',
  							    ...
  							    onLoadSuccess:function(){
  							    	if('${user.currentProvince}'!=''){
  							    		jQuery('#studentForm').find('#currentProvince').combobox('setValue','${user.currentProvince}');
  									}
  							    	if('${user.currentCity}'!=''){
  							    		jQuery('#studentForm').find('#currentCity').combobox('setValue','${user.currentCity}');
  									}
  							    	if('${user.currentCounties}'!=''){
  							    		jQuery('#studentForm').find('#currentCounties').combobox('setValue','${user.currentCounties}');
  									}
  							    },
  							    onChange:function(newValue, oldValue) {
  							    	if(newValue!=undefined&&""!=newValue) {
  							    		var url = '${ctx}/common/regions/getNextRegionsByCode.do?regionsCode='+newValue;
  								    	jQuery('#studentForm').find('#currentCity').combobox("reload",url);
  								    	jQuery('#studentForm').find('#currentCity').combobox('setValue','');
  								    	jQuery('#studentForm').find('#currentCounties').combobox('setValue','');
  							    	}
  							    }
  							}); 
			...
			<tr>
				...
				<td>
					<select id="expertType" name="expertType" class="easyui-combotree" checkbox="true" style="width: 158px;" editable="false">
							...
							<script type="text/javascript">
                             jQuery("#studentForm").find("#expertType").combotree({
								...
								onLoadSuccess:function(){
									...
									value = value.replace(/\"/g,'').replace(/\[/g,'').replace(/\]/g,'');
									if(value!=''){
										  jQuery("#studentForm").find("#expertType").combotree('setValues',value);
									}
								 }
							}); 
                            </script>
				...
				<td>
					<select id="expertLevel" name="expertLevel" class="easyui-combotree" style="width: 158px;" editable="false">
							...
							<script type="text/javascript">
                             jQuery("#studentForm").find("#expertLevel").combotree({
								...
								onLoadSuccess:function(){
									...
									value = value.replace(/\"/g,'').replace(/\[/g,'').replace(/\]/g,'');
									if(value!=''){
										  jQuery("#studentForm").find("#expertLevel").combotree('setValues',value);
									}
								 }
							}); 
                            </script>
			...
			<tr>
				...
				<td>
					...
					<td>人才类型</td>
					<td><select name="isPerson" class="easyui-combobox" 
						style="width: 158px;"  editable="false">
						<option value=""></option>
							${dict:getEntryOptionsSelected('IS_PERSON',user.isPerson) }
					</select>
					</td>
				</tr>
				<tr>
				<td>传真</td>
				<td>
					<input type="text" class="easyui-textbox" name="faxes" value="${user.faxes}" />
				</td>
				</tr>
			<tr>
				<td>个人简介</td>
...
function changePaperworkType() {
	var paperworkType = $("#paperworkType","#studentForm").combobox("getValue");
	if("01"==paperworkType) {//居民身份证
		$("#paperworkNo","#studentForm").textbox({
			validType:'idcard'
		});
	} else {
		$("#paperworkNo","#studentForm").textbox({
						...
						if("200"==responseCode) {//成功
							jQuery.messager.alert("提示信息","保存成功","info",function() {
								...
								$('#showAddDialog').window({
									  title:'修改人才信息',
									  href:'${ctx}/common/user/student/editTabs.do?id='+responseMsg
								});
							});
						} else {//错误
