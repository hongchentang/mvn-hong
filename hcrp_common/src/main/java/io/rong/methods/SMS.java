...
public class SMS {
	 ...
	 * @param  mobile:接收短信验证码的目标手机号，每分钟同一手机号只能发送一次短信验证码，同一手机号 1 小时内最多发送 3 次。（必传）
	 * @param  templateId:短信模板 Id，在开发者后台->短信服务->服务设置->短信模版中获取。（必传）
	 * @param  region:手机号码所属国家区号，目前只支持中图区号 86（必传）
	 ...
	 * @param  p3:短信模板中，自定义变量值，如果在通知短信模板中定义了 {p3} 则在发送通知短信时必须传入此参数，替换模板中的 {p3}，否则此参数可以不传。
	 *
	 * @return SMSSendCodeReslut
	 **/
	public SMSSendCodeReslut sendNotify(String mobile, String templateId, String region, String p1, String p2, String p3) throws Exception {
		if (mobile == null) {
			throw new IllegalArgumentException("Paramer 'mobile' is required");
		}
	    ...
	    if (p1 != null) {
	    	sb.append("&p1=").append(URLEncoder.encode(p1.toString(), UTF8));
	    }
	    
	    if (p2 != null) {
	    	sb.append("&p2=").append(URLEncoder.encode(p2.toString(), UTF8));
	    }
	    
	    if (p3 != null) {
	    	sb.append("&p3=").append(URLEncoder.encode(p3.toString(), UTF8));
	    }
