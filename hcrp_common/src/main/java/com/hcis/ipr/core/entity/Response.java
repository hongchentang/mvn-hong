package com.hcis.ipr.core.entity;
...
public class Response implements Serializable {
	...
	private static final long serialVersionUID = -257962938367851276L;
	...
	public static Response defaultInstance(int updateRecCount) {
		if (updateRecCount > 0) {
	...
	public static Response newDefaultResponse(int updateRecCount) {
		if (updateRecCount == 100) {
			return new Response(300, "添加邮箱不能超过3个!");
		} else if (updateRecCount > 0) {
			return new Response(200, "操作成功!");
		} else if (updateRecCount == 0) {
			return new Response(300, "该邮箱已存在!");
		} else {
	 ...
	 * @return
	 */
	public static Response newResponse(int count) {
		if (count > 0) {
	 ...
	 * @return
	 */
	public static Response newResponse(String errorMsg) {
		if (StringUtils.isEmpty(errorMsg)) {
			return Response.successInstance();
		} else {
			return new Response("01", errorMsg);
		}
	}
}
