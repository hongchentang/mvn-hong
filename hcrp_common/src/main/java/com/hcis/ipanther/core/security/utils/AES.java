 ...
 * 提供AES的所有核心加密解密。
 * <p />
 * 使用 {@code AES/CBC/NoPadding} 模式时，解密后的原文后会有多余的空格字符需要去除。
 * <p />
 * <b>该类是线程安全的。</b>
 * 
 * @author fuchun
...
public class AES {
    ...
    /**
     * 根据给定的私密密钥和默认的 {@code AES/ECB/PKCS5Padding}填充方式，获得一个 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @return 给定的私密密钥和默认的 {@code AES/ECB/PKCS5Padding}填充方式的 {@code Rijndael}。
     */
    public static AES getInstance(byte[] key) {
        return getInstance(key, AESMode.ECB_PKCS5);
    }

    /**
     * 根据给定的私密密钥和给定的填充方式，获得一个 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param mode 模式与填充方式。
     * @return 给定的私密密钥和给定的填充方式的 {@code Rijndael}。
     */
    public static AES getInstance(byte[] key, AESMode mode) {
        final String hexKey = Hex.encodeHexString(key);
        final String cacheKey = StringUtils.join(new Object[] {
                hexKey, StringUtils.replace(mode.getName(), "/", "_")
        }, "_");
        if (AES_CACHES.containsKey(cacheKey)) {
            return AES_CACHES.get(cacheKey);
        }
        AES o = new AES(key, mode);
        AES_CACHES.put(cacheKey, o);
        return o;
    }

    /**
     * 根据给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式，获得一个 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param iv IV 向量。
     * @return 给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式的 {@code Rijndael}。
     */
    public static AES getInstance(byte[] key, byte[] iv) {
        return getInstance(key, iv, AESMode.CBC_PKCS5);
    }

    /**
     * 根据给定的私密密钥、IV向量和指定的填充方式，获得一个 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param iv IV向量。
     * @param mode 模式与填充方式。
     * @return 返回给定的私密密钥、IV向量和指定的填充方式的 {@code Rijndael}。
     */
    public static AES getInstance(byte[] key, byte[] iv, AESMode mode) {
        final String hexKey = Hex.encodeHexString(key);
        final String hexIV = Hex.encodeHexString(iv);
        final String cacheKey = StringUtils.join(new Object[] {
                hexKey, hexIV, StringUtils.replace(mode.getName(), "/", "_")
        }, "_");
        if (AES_CACHES.containsKey(cacheKey)) {
            return AES_CACHES.get(cacheKey);
        }
    ...
    /**
     * 根据给定的私密密钥和默认的 {@code AES/ECB/PKCS5Padding}填充方式，构造一个新的 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     */
    public AES(byte[] key) {
        this(key, AESMode.ECB_PKCS5);
    }

    /**
     * 根据给定的私密密钥和给定的填充方式，构造一个新的 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param mode 模式与填充方式。
     */
    ...
    /**
     * 根据给定的私密密钥、IV向量和默认的 {@code AES/CBC/PKCS5Padding} 填充方式，构造一个新的 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param iv IV 向量。
     */
    public AES(byte[] key, byte[] iv) {
        this(key, iv, AESMode.CBC_PKCS5);
    }

    /**
     * 根据给定的私密密钥、IV向量和指定的填充方式，构造一个新的 {@code Rijndael}。
     * 
     * @param key 私密密钥。
     * @param iv IV向量。
     * @param mode 模式与填充方式。
     */
    public AES(byte[] key, byte[] iv, AESMode mode) {
        validateKey(key);
        validateIV(iv);
    ...
    private void validateKey(byte[] key) {
        if (key == null || key.length < KEY_LENGTH) {
            throw new IllegalArgumentException(String.format("Invalid AES key length: %s bytes",
    ...
    private void validateIV(byte[] iv) {
        if (iv == null || iv.length < IV_LENGTH) {
            throw new IllegalArgumentException(String.format("Wrong IV length: must be %s bytes long",
                    IV_LENGTH));
        }
    }
    ...
    public byte[] encryptString(String input, String encoding) {
        ...
        if (StringUtils.isBlank(encoding)) {
            encoding = CharEncoding.ISO_8859_1;
        }
        byte[] data;
        try {
    ...
    private void initEncryptionCipher() {
        if (enc == null) {
            try {
                ...
                LOGGER.error(String.format("The current platform no such algorithm - %s, Cause: %s",
                        ALGORITHM, ex.getMessage()));
            } catch (NoSuchPaddingException ex) {
                LOGGER.error(String.format("The current platform no such padding - %s, Cause: %s",
                        mode.getName(), ex.getMessage()));
            } catch (InvalidKeyException ex) {
                LOGGER.error(StringUtils.join(new Object[] {
                        "Invalid key - ", (keySpec != null ? keySpec.toString() : "encrypt key. "),
                        " Cause: ", ex.getMessage()
                }));
    ...
    private void initDecryptionCipher() {
        if (dec == null) {
            try {
                ...
                LOGGER.error(StringUtils.join(new Object[] {
                        "Invalid key - ", (keySpec != null ? keySpec.toString() : "decrypt key. "),
                        " Cause: ", ex.getMessage()
                }));
            } catch (InvalidAlgorithmParameterException ex) {
                LOGGER.error(StringUtils.join(new Object[] {
    ...
    public static void main(String[] args) throws Exception {
    	long cm =System.currentTimeMillis()/1000;
        final String encoding = "UTF-8";
        final String plain = "{\"userId\":\"a2d1537fff8a49e5ad326e4e9c140307\",\"relationId\":\"testClass\",\"currentTime\":\""+cm+"\"}";
        ...
        //final byte[] key =  DigestUtils.md5("0123456789ABCDEF");
        ...
        System.out.println("");
        AES cbc_aes = new AES(key,iv,  AESMode.CBC_NO);
        System.out
                .println("CBC+++++++++++++++++++++++++++++++++ Base64 ++++++++++++++++++++++++++++++++++++");
        ...
        AES cbc_aes_pkcs5 = new AES(key, iv, AESMode.CBC_PKCS5);
        System.out
