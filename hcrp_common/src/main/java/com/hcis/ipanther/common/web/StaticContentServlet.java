package com.hcis.ipanther.common.web;
...
public class StaticContentServlet extends HttpServlet {
	...
	public static final String CONTENTTYPE_WEB="web";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 根据参数类型web(基于项目路径),file(配置读取文件路径)获取请求内容的基本信息.
		String contentType = request.getParameter("contentType");
		if(StringUtils.equals(StaticContentServlet.CONTENTTYPE_WEB,contentType)){
			contentInfo = getContentInfo(contentPath);
		}
		else if(StringUtils.equals(StaticContentServlet.CONTENTTYPE_FILE,contentType)){
			contentInfo = getContentInfoFile(contentPath);
		}
		else{
			contentInfo = getContentInfo(contentPath);
		}
	...
	/**
	 * 创建Content基本信息.本地文件路径
	 */
	private ContentInfo getContentInfoFile(String contentPath) {
		ContentInfo contentInfo = new ContentInfo();
		String rootDirectory = CommonConfig.getProperty("attachment.root.dir");
		
		String realFilePath = rootDirectory + contentPath;
		File file = new File(realFilePath);
